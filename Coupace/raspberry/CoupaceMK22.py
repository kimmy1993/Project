...
pygame.init()
pygame.mixer.music.load("Camera.wav")
...
class PageCam(Page):
    ...
    def __init__(self, *args, **kwargs):
        ...
        btnShutter = tk.Button(self, text = "촬영", font = (15), command = self.photoShoot, bg = cCOU)
        ...
        self.entryIDList.place( width = 200, height = 100 , x = 300 ,y = 110 )

        #결제버튼
        self.btnSaveMileage = tk.Button(self, text = "결제", font = (15), command = self.sendIDPlusPrice, bg = cCOU)
        ...
        #결제, 적립여부
        self.var = IntVar()
        self.radioSave = tk.Radiobutton(self, bg = cCOU, text = "포인트적립", variable = self.var, value = 0, font = (15), borderwidth = 0, highlightthickness = 0)
        self.radioSave.place( width = 120, height = 30, x = 270, y = 300)
        self.radioUse = tk.Radiobutton(self, bg = cCOU, text = "포인트사용", variable = self.var, value = 1, font = (15), borderwidth = 0, highlightthickness = 0)
        self.radioUse.place( width = 120, height = 30, x = 270 , y = 350)
        ...
        #self.entryInputID = tk.Entry(self, textvariable = self.entrySentence, bg = "#9BEEBE", font = (15))
        #self.entryInputID.place( width = 200, height = 30, x = 300, y = 220)
        #self.entryInputID.config( state = "disabled")
        #self.entryInputID.lower(self)

        #안내라벨
        self.lblInfo = tk.Label(self, text = "카메라앞에 위치하고 촬영하세요", font =(15), bg = cCOU)
        self.lblInfo.place( width = 300, height = 30, x = 250, y = 400)
    ...
    def photoShoot(self):
        ...
        #1. 사진찍기
        pic,dum, smile=cv.cv(1)
        pygame.mixer.music.play()

        #self.sendImage() #2. 사진전송
        #self.sendTotalPrice() #계산정보 전송
        """PlaySound('Camera Shutter Click-SoundBible.com-228518582.wav', SND_ASYNC)
        if (smile==1):
            messagebox.showinfo("카메라","smile point")
        
        if (pic==0):
            messagebox.showinfo("카메라", "얼굴 인식에 실패하셧습니다")
        """
        if pic == 0:
            self.lblInfo.config( text = "얼굴 인식에 실패했습니다")
        if not(pic==0):
            self.lblInfo.config( text = "포인트적립 포인트사용 선택")
            self.receiveID(pic, smile)#3. ID 3개받기
            #self.sendIDPlusPrice()#4. ID 1개 + 계산정보 전송
            #self.entryInputID.config( state = "normal")
    ...
    def receiveID(self, pic, smile):
        ...
        message = "COPIC" + "@@" + MasterID + "@@"+str(pic)+"@@"+str(smile)+"@"
    ...
    def sendIDPlusPrice(self):
        ...
        #if self.entrySentence.get() != None:
        ...
        #else:
        #message = "COUSE" + "@@" + MasterID + "@@" + self.entrySentence.get() + "@@" + (str)(self.var.get()) + "@@" + (str)(self.totalPrice) + "@"
        ...
        if data == 'success':
            print("COUSE success")
            self.entryIDList.delete(0, END) # 기존리스트박스항목 정리
            self.lblInfo.config(text="결제가 완료되엇습니다")
...
class Page1(Page):
   ...
   def __init__(self, *args, **kwargs):
    ...
    """
       ...
       buttonPay = tk.Button(self, text ="결제", bg = "#4980B3", command = self.showMessageBox, font = ("a발레리나", 15))
...
"""
   ...
   def buttonLoad(self):
           #self.frameButtonGrid = tk.Frame(self, width = 400, height = 200, bg = "Red")
           self.frameButtonGrid = tk.Frame(self, width = 400, height = 200, bg = cCOU)
...
class Page3(Page):
   ...
   def join(self):
       ...
       if (self.sentence1.get() != "" and self.sentence2.get() != ""
           ...
           message = 'ADDUS' + '@@' + self.sentence1.get() + '@@' + self.sentence3.get()+ '@@' + self.sentence2.get() + '@@' + self.sentence4.get() + '@@' + self.familyID + '@@' +str(pic_insert)+"@"
        
           s.send(message.encode())
           msg=s.recv(2).decode()
           if (msg==str(1)):
               for i in range(pic_insert):
                   paths="./insert/0.png"
                    #cv2.imwrite(paths,result[i])
                   s_i=open(paths,'rb')
...
class PageLogin(Page):
    ...
    #def __init__(self, button):
        ...
        buttonOk = tk.Button(self, text = "로그인", bg = "#4980B3", command = self.checkID2, font=(15) )
    ...
    def checkID(self):
        ...
        #play = lambda: PlaySound('Camera Shutter Click-SoundBible.com-228518582.wav', SND_FILENAME)
        #PlaySound('Camera Shutter Click-SoundBible.com-228518582.wav', SND_FILENAME)
        #PlaySound('Camera Shutter Click-SoundBible.com-228518582.wav', SND_ASYNC)
        #Beep('99', '10')
