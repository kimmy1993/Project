...
class PageCam(Page):
    ...
    def __init__(self, *args, **kwargs):
        ...
        label = tk.Label(self, text="This is cam page", bg = "#4980B3")
        label.place(width = 200, height = 50, x = 100, y = 300)

        btnShutter = tk.Button(self, text = "촬영", command = self.photoShoot)
        btnShutter.place(x = 100, y = 300, width = 100, height = 50)

        self.lblTotalPrice = tk.Label(self, text = "Total : ", bg = "#4980B3" )
        self.lblTotalPrice.place(width = 200, height = 50, x = 200, y = 300 )
...
class Page1(Page):
   ...
   def __init__(self, *args, **kwargs):
       ...
       label = tk.Label(self, text="This is page 1", bg = "#4980B3")
       label.pack(side="top", fill="both", expand=True)
    ...
    """
       ...
       buttonPay = tk.Button(self, text ="결제", bg = "#4980B3", command = self.showMessageBox)
       buttonPay.place(width = 100, height = 50, x = 100, y = 350 )
...
"""
       ...
       self.itemOrderBox.place(width = 150, height = 200, x = 150, y = 100)
       ...
       #총금액레이블
       self.lblTotalPrice = tk.Label(self, text = "total: 0", bg = "#4980B3")
       self.lblTotalPrice.place(width = 200, height = 50, x = 50, y = 300)
           ...
           for row in range( 4 ):
              for col in range( 4 ):
                   if self.count < len(itemList):
                       ...
                       button = tk.Button(self.frameButtonGrid, text = buttonTxt, width = 8, height = 5, command = lambda indexer = indexer: self.addListBox( indexer), wraplength=80, justify=LEFT ) #wraplength = 80
...
class Page3(Page):
   ...
   def join(self):
       ...
       if (self.sentence1.get() != "" and self.sentence2.get() != ""
           ...
           s = socket.socket(socket.AF_INET, socket.SOCK_STREAM) #소켓생성
           s.connect((HOST,PORT))
           if (self.sentence5.get() != ""):
               self.familyID = self.sentence5.get()
           else:
               self.familyID = self.sentence1().get()
...
class PageLogin(Page):
    ...
    def checkID2(self):
        ...
        message = 'LOGIN' + '@@' + 'sejong' + '@@' + '1234' + '@@' + '1' + '@'
        message2 = 'LOGIN' + '@@' + self.sentence1.get() + '@@' + self.sentence2.get() + '@@' + '1' + '@'
        #s.send(message)
